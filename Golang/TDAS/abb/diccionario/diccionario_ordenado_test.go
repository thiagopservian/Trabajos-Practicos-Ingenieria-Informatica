...
func TestRandomBorradosNoGeneraStackOverflow(t *testing.T) {
	dic := TDADiccionario.CrearABB[int, int](func(i1, i2 int) int { return i1 - i2 })
	arr := make([]int, 0)
	for i := 0; i < 1000; i++ {
		randomNumber := rand.Intn(1000) + 1 // Genera un número aleatorio entre 1 y 1000
		if !dic.Pertenece(randomNumber) {
			dic.Guardar(randomNumber, randomNumber)
			arr = append(arr, randomNumber)
		}
	}
	for i := 0; i < len(arr); i++ {
		randomNumber := arr[i]
		require.EqualValues(t, randomNumber, dic.Borrar(randomNumber))
	}
	require.EqualValues(t, 0, dic.Cantidad())
}
